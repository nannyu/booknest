...
| **NeoDB** | 中文简介 / 出版社 / 译者 | ✅ 已实现 | 开源联邦目录，搜索 API 公开免 OAuth |
...
启用方式：在 `.env` 中设置 `ENABLE_CROSSREF=true` / `ENABLE_LOC=true` / `ENABLE_NEODB=true` 即可。商业 ISBN 服务额外需要 `COMMERCIAL_ISBN_PRESET` + `COMMERCIAL_ISBN_API_KEY`（详见 [docs/provider-policy.md](docs/provider-policy.md)）。其他 Provider 已在 `config/providers.ts` 中预留位置，待上游接口稳定后再补实现。
